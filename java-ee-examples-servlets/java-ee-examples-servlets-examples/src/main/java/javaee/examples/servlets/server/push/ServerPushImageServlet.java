/**
 * @Class ServerPushImageServlet
 * @author Hector
 * @Created on Jun 22, 2019, 9:03:22 PM
 */
...
@WebServlet(name = "ServerPushImageServlet", urlPatterns = {"/ServerPushImageServlet"})
public class ServerPushImageServlet extends HttpServlet {
    ...
    private void doRequest(HttpServletRequest request, HttpServletResponse response) throws IOException {
            ...
            if (pb != null) {
                for (int row = 0; row < 5; row++) {
                    for (int col = 0; col < 8; col++) {
                        pb.path("images/keyboard_buttons/keyboard_buttons-" + row + "-" + col + ".jpeg")
        ...
        try (PrintWriter writer = response.getWriter()) {
            ...
            for (int row = 0; row < 5; row++) {
                ...
                for (int col = 0; col < 8; col++) {
                    html.append("    <td>");
                    html.append("<img src='images/keyboard_buttons/keyboard_buttons-" 
                            + row + "-" + col + ".jpeg' style='width:100px; height:106.25px;'>");
                    html.append("    </td>");
                }
                html.append("  </tr>");
            }
