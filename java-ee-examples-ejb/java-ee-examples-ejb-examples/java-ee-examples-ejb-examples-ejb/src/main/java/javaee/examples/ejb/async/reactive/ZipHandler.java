...
public class ZipHandler {
    ...
    public Future<String> handler(FileBean file) throws IOException {
        return new AsyncResult(FileSystemUtils.save(file.getFile(), "zip", "zip_" + new Date().getTime() + ".zip"));
    }
    
    public String handlerCs(FileBean file) throws IOException {
        return FileSystemUtils.save(file.getFile(), "zip", "zip_" + new Date().getTime() + ".zip");
    }
}
